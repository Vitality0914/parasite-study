...
이 폴더 하나에 시작 화면, 1번 학명쓰기(연습), 2번 학명쓰기(실전), 3번 기생충 꼬맨틀, 그리고 각 앱의 데이터가 모두 포함되어 있습니다.

## 구조

- `index.html`: 학명쓰기 연습·실전·꼬맨틀 시작 화면
- `typing-practice/`: 1번 학명쓰기(연습) 전체 파일
- `typing/`: 2번 학명쓰기(실전) 전체 파일
- `semantle/`: 3번 기생충 꼬맨틀 전체 파일
...
세 앱은 외부 서버나 API 없이 정적 HTML·CSS·JavaScript·데이터 파일만으로 작동합니다.
